...
async function missingIds(
  db: D1Database,
  table: "products" | "affiliate_profiles",
  ids: string[],
): Promise<string[]> {
...
export class ReferentialError extends Error {
  constructor(
    message: string,
    public readonly missing: { products: string[]; profiles: string[] },
  ) {
    super(message);
...
/** Throw if any referenced product/profile id is absent. */
export async function assertReferences(
  db: D1Database,
  refs: { productIds?: string[]; profileIds?: string[] },
): Promise<void> {
